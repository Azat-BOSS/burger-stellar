...
const burgerConstructor = createSlice({
  ...
  reducers: {
    getConstructorElements: (state, action) => {
      state.constructorArr = [...state.constructorArr, {...action.payload, id: uuidv4()}]
    },
    getConstructorBuns: (state, action) => {
      state.bunArr = action.payload
    },
    getTotalPrice: (state) => {
      state.price = state.constructorArr.reduce((prev, curr) => curr.type === "bun" ? (prev + curr.price) : prev + curr.price, 0)
    },
    removeConstructorElement: (state, action) => {
      state.constructorArr = state.constructorArr.filter(el => el.id !== action.payload)
    },
